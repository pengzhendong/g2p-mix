...
class ToneSandhi:
    ...
    def _merge_continuous_three_tones(self, tokens):
        ...
        for i, token in enumerate(tokens):
            if (
                ...
                new_tokens[-1].push_back(token)
                # e.g. 也 很 好 => 也很好 merge twice when length less than 3
                if len(new_tokens[-1].word) >= 3:
                    merge_last[i] = True
            else:
                new_tokens.append(token)
        return new_tokens
    ...
    def _neural_sandhi(self, token):
        ...
        # 徐世荣等语言学家说：“至于一部分书面语言中的叠字形容词，就决不能变。”
        if (
            self._is_reduplication(token.word)
            and token.pos[0] in {"n", "v"}
        ...
        if (
            (
                token.word[-1] in "们子"
                and token.pos[0] in {"r", "n"}
                and token.word not in self.must_not_neural_tone_words
            )
            # e.g. 桌上, 地下, 家里
            or (
                len(token.word) == 2
                and token.word[-1] in "上下里"
                and token.pos[0] in {"s", "l", "f"}
            )
            # e.g. 上来, 下去
            or (
                len(token.word) == 2
                and token.word[-2] in "上下进出回过起开"
                and token.word[-1] in "来去"
            )
