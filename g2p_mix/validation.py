...
from .models import Language, PhoneAlphabet, Pronunciation, PronunciationUnit, Span, TextToken
from .phonetics import canonical_pinyin_phones, split_jyutping, split_pinyin, transcribe_arpabet_phone
from .renderers import NativeRenderer
...
def validate_prediction(
    *,
    ...
    """Validate the public pronunciation mapping contract."""
    ...
    for token_id, pronunciation in result.items():
        ...
        if pronunciation.token_id != token_id:
            raise AlignmentError(f"{producer} token_id mismatch for mapping key {token_id}: {pronunciation.token_id}")
        ...
        for unit in pronunciation.units:
            _validate_unit_structure(producer, token_id, unit)
            ...
            _validate_unit_phonetics(producer, token_id, unit)

        if next_position != len(occurrences):
            raise AlignmentError(f"{producer} source coverage mismatch for token {token_id}")


def validate_processor_identity(
    *,
    producer: str,
    baseline: Mapping[int, Pronunciation],
    result: Mapping[int, Pronunciation],
) -> None:
    """Ensure a processor changes phonetics without changing alignment identity."""
    ...
    for token_id, before in baseline.items():
        ...
        before_alignment = tuple(
            (unit.text, unit.source_spans, unit.alphabet, unit.is_unknown) for unit in before.units
        )
        after_alignment = tuple((unit.text, unit.source_spans, unit.alphabet, unit.is_unknown) for unit in after.units)
...
def _validate_unit_structure(producer: str, token_id: int, unit: object) -> None:
    ...
    if not isinstance(unit.is_unknown, bool):
        raise AlignmentError(f"{producer} returned an invalid is_unknown flag for token {token_id}")
    if not isinstance(unit.phones, tuple):
    ...
    if unit.is_unknown:
        if (
            unit.phones
            or unit.native
            or unit.tone is not None
            or unit.source_phones
            or unit.tone_contour
            or unit.stress_marks
        ):
            raise AlignmentError(f"{producer} returned phonetic data for unknown unit in token {token_id}")
    elif not unit.phones:
        raise AlignmentError(f"{producer} returned empty phones for known unit in token {token_id}")
    ...
    if not isinstance(unit.tone_contour, tuple) or any(
        isinstance(value, bool) or not isinstance(value, int) or value not in range(1, 6) for value in unit.tone_contour
    ):
...
def _validate_unit_phonetics(producer: str, token_id: int, unit: PronunciationUnit) -> None:
    if unit.is_unknown:
        return
    if unit.alphabet is PhoneAlphabet.ARPABET:
        ...
        if any(phone[-1].isdigit() for phone in unit.phones):
            raise AlignmentError(f"{producer} returned stressed ARPABET instead of base phones for token {token_id}")
